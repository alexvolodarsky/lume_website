...
var LayerSlider = {
	...
	generatePreview : function(index) {
		...
		jQuery('#ls-layers .ls-layer-box').eq(index).find('.ls-sublayers > tr').each(function() {
			...
			var setPositions = function(){
				...
				if(top.indexOf('%') != -1) {
					item.css({ top : draggable.height() / 2 - item.height() / 2 - pt - bt });
				} else {
					item.css({ top : parseInt(top) });
				}

				if(left.indexOf('%') != -1) {
					item.css({ left : draggable.width() / 2 - item.width() / 2 - pl - bl });
				} else {
	...
	dragging : function() {
		...
		// Set positions
		wrapper.find('input[name="top"]').eq(index).val(top);
		wrapper.find('input[name="left"]').eq(index).val(left);
	},
	...
	play : function( index ) {
		...
		var skinPath = pluginPath + jQuery('.ls-settings input[name="skinspath"]').val();
				...
				jQuery.ajax(jQuery(el).attr('action'), {
					...
					complete : function() {
						...
						if(jQuery('#ls-layers .ls-layer-box').length == LayerSlider.counter) {
							...
							if(document.location.href.indexOf('action=add') != -1) {

								// Get ID
								var id = jQuery('input[name="sliderkey"]').val();
...
jQuery(document).ready(function() {
	...
	jQuery('.ls-settings input[name="width"], .ls-settings input[name="height"]').keyup(function() {
		jQuery('#ls-layers .ls-layer-box').each(function( index ) {
			LayerSlider.generatePreview(index);
		});
	});
	...
	// Duplicate layer
	jQuery('.ls-slide-options a.duplicate').live('click', function(e){
		e.preventDefault();
	...
	// Duplicate sublayer
	jQuery('.ls-sublayer-options a.duplicate').live('click', function(e) {
		e.preventDefault();
	...
	// Sublayer: Style
	jQuery('.ls-sublayers input[name="top"], .ls-sublayers input[name="left"], .ls-sublayers textarea[name="style"]').live('keyup', function() {
